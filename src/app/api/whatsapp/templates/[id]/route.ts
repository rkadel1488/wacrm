 ...
 *
 * Templates are a local-only saved-message catalog (no Meta approval
 * flow) — PATCH and DELETE just edit/remove the local row.
 */
...
export async function PATCH(
  ...
  try {
    ...
    if (updErr) {
      return NextResponse.json(
        { error: `Failed to save changes: ${updErr.message}` },
        { status: 500 },
      )
    }
