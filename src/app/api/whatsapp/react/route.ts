...
import { sendReaction } from '@/lib/whatsapp/bridge-api'
 ...
 *
 * Sends the reaction via the bridge and mirrors it into
 * `message_reactions` (delete on empty emoji). Customer-side reactions
 * are handled by the bridge webhook — this route only writes
...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      console.error('[whatsapp/react] bridge send failed:', message)
      return NextResponse.json(
